...
async function main() {
  ...
  try {
    ...
    // Insert all stations one by one to avoid timeout issues
    const stationEntries = Array.from(uniqueStations.entries());
    let insertedCount = 0;
    
    for (const [stationId, stationInfo] of stationEntries) {
      try {
        ...
        if (insertedCount % 50 === 0) {
          console.log(`✅ Inserted ${insertedCount}/${stationEntries.length} stations`);
        }
      } catch (error) {
        console.error(`❌ Error inserting station ${stationId}: ${error}`);
      }
    }
    
    console.log(`✅ Successfully inserted ${insertedCount} stations`);
          ...
          for (const listing of sampleListings) {
            ...
            await prisma.apartment.create({
              data: {
                ...
                features: listing.features ? JSON.stringify(listing.features.split(' · ')) : null,
